...
class AlgoTuneTaskAdapter:
    ...
    def __init__(self, algotune_path: Optional[str] = None, task: Optional[str] = None):
        ...
        if algotune_path is None:
            raise ValueError(
                "Please specify algotune_path to the AlgoTune repository directory."
            )
        ...
        if not self.algotune_tasks_path.exists():
            raise ValueError(
                f"AlgoTuneTasks directory not found at: {self.algotune_tasks_path}"
            )
    ...
    def _load_tasks(self):
        ...
        print(
            f"Loaded {len(self.available_tasks)} tasks from {self.algotune_tasks_path}"
        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if not any(
                        x in import_str for x in ["AlgoTune", "register_task", "Task"]
                    ):
                    ...
                    for item in node.body:
                        if isinstance(item, ast.FunctionDef) and item.name in [
                            ...
                            try:
                                ...
                                method_source_lines = source_lines[
    ...
    def _harmonize_solve_and_is_solution(
        ...
        if (
            "transformed_image" in harmonized_is_solution
            and task_name == "affine_transform_2d"
        ):
...
"""
        ...
        if (
            "operands could not be broadcast" in task_name
            or task_name == "affine_transform_2d"
        ):
...
"""
    ...
    def _generate_initial_program(self, task_name: str) -> str:
        ...
        unique_imports = [
            imp for imp in unique_imports if not imp.startswith("from typing import")
        ]
        ...
        if solve_method:
            ...
            method_body = self._generate_task_specific_method(
                task_name, solve_method, class_info
            )
        ...
        if solve_method and is_solution_method:
            method_body, is_solution_method_body = (
                self._harmonize_solve_and_is_solution(
        ...
        docstring_description = re.sub(
            r"b\\\\x[0-9a-fA-F]{2}", "b'\\\\x00'", docstring_description
        )
        docstring_description = re.sub(
            r"\\\\x[0-9a-fA-F]{2}", "\\\\x00", docstring_description
        )

        # Fix any remaining problematic patterns
        docstring_description = docstring_description.replace(
            "\\\\xencrypted", "\\\\x00encrypted"
        )
        docstring_description = docstring_description.replace(
...
{return_code}"""
    ...
    def create_task_files(
        self, task_name: str, output_dir: Optional[str] = None
    ) -> str:
        """
