...
class TestMigrationNoDuplicates(unittest.TestCase):
    ...
    def test_migration_creates_clean_uuid_ids(self):
        ...
        for island in range(3):
            for i in range(3):
                ...
                self.db.add(prog)
                self.db.island_generations[island] = (
                    3  # Set generation to trigger migration
                )
    ...
    def test_migration_target_islands_are_different(self):
        ...
        migrant_suffix_programs = [
            pid for pid in self.db.programs.keys() if "_migrant_" in pid
        ]
        self.assertEqual(
            len(migrant_suffix_programs),
            0,
            "No programs should have _migrant_ suffixes",
        )
    ...
    def test_migration_with_feature_map_conflicts_resolved_cleanly(self):
        ...
        prog1 = self._create_test_program(
            "high_quality", 0.9, [0.5, 0.5], island=0, generation=3
        )
        prog2 = self._create_test_program(
            "low_quality", 0.3, [0.5, 0.5], island=1, generation=3
        )
    ...
    def test_migration_uses_map_elites_deduplication(self):
        ...
        self.assertEqual(
            coords_low,
            coords_high,
            "Programs with same custom metrics should map to same cell",
        )
        ...
        self.assertEqual(
            cell_program_id,
            "high_score",
            "Better program should be kept in MAP-Elites cell",
        )
        ...
        if migrant_in_feature_map:
            ...
            self.assertEqual(
                migrant_program.metrics["combined_score"],
                0.9,
    ...
    def test_migration_skips_duplicate_code_on_target_island(self):
        ...
        island_1_before = len(
            [pid for pid in self.db.islands[1] if pid in self.db.programs]
        )
