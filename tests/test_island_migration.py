...
class TestIslandMigration(unittest.TestCase):
    ...
    def test_migration_ring_topology(self):
        ...
        self.assertEqual(
            len(migrant_suffix_ids), 0, "No programs should have _migrant_ suffixes"
        )
        ...
        for island_idx, island_map in enumerate(self.db.island_feature_maps):
            programs_in_islands.extend(
                [(pid, island_idx) for pid in island_map.values()]
            )
        ...
        self.assertGreater(
            len(islands_with_programs),
            1,
            "Migration should distribute programs across islands",
        )
    ...
    def test_migration_preserves_best_programs(self):
        ...
        self.assertIsNotNone(
            high_score_program, "Original high score program should still exist"
        )
